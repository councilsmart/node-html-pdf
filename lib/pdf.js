...
try {
  ...
  console.log(
    'html-pdf: Failed to load PhantomJS module. Using specified one specified with "phantomPath"'
  );
}
...
PDF.prototype.toBuffer = function PdfToBuffer(callback) {
  this.exec(function execPdfToBuffer(err, res) {
    console.log("response from html-pdf", res);
    console.log("error from html-pdf", err);

    if (err) return callback(err);

    if (!res) {
      return callback(new Error("html-pdf: PDF generation failed due to an unknown reason"));
    }
...
PDF.prototype.exec = function PdfExec(callback) {
  ...
  function respond(code, err, data) {
    ...
    // an error occured.
    if (
      (typeof code !== "undefined" && code !== null && code !== 0) ||
      err ||
      (code === 0 && !data)
    ) {
      var error = null;
